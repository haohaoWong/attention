...
# 得到高斯金字塔和高斯差分金字塔
def get_DoG(img, n, sigma0, S=None, O=None):
    ...
    sample = [under_sampling(img, 1 << o) for o in range(O)]  # 降采样取图片作为该层的输入
...
# ：通过泰勒展开精调位置精调位置
def adjust_LocalExtrema(DoG, o, s, x, y, contrastThreshold, edgeThreshold, sigma, n, SIFT_FIXPT_SCALE):
...
def get_MainDirection(img, r, c, radius, sigma, BinNum):
...
# 关键点定位
def Locate_KeyPoint(DoG, sigma, GuassianPyramid, n, BinNum=36, contrastThreshold=0.04, edgeThreshold=10.0):
            ...
            for i in range(img.shape[0]):
                for j in range(img.shape[1]):
                    ...
                    if np.abs(val) > threshold and \
                        ...
                        point, x, y, layer = adjust_LocalExtrema(DoG, o, s, i, j, contrastThreshold, edgeThreshold,
                                                                 sigma, n, SIFT_FIXPT_SCALE)
                        if point == None:
                            continue
                        scl_octv = point[-1] * 0.5 / (1 << o)
                        # GetMainDirection：（确定极值点的位置以后就）求主方向
                        omax, hist = get_MainDirection(GuassianPyramid[o][layer], x, y,
...
# 更小的计算描述符函数
def get_SIFTDescriptor(img, ptf, ori, scl, d, n, SIFT_DESCR_SCL_FCTR=3.0, SIFT_DESCR_MAG_THR=0.2,
...
def cal_Descriptors(gpyr, keypoints, SIFT_DESCR_WIDTH=4, SIFT_DESCR_HIST_BINS=8):
    ...
    for i in range(len(keypoints)):
        ...
        descriptors.append(get_SIFTDescriptor(img, ptf, kpt[-1], size * 0.5, d, n))  # calcSIFTDescriptor：更小的计算描述符函数
    return descriptors
...
def SIFT(img, showDoGimgs=False):
    ...
    DoG, GuassianPyramid,octaves = get_DoG(img, n, sigma0)  # 函数1.1，getDoG：得到高斯金字塔和高斯差分金字塔
    ...
    #2. 确定关键点位置,为关键点赋予方向
    KeyPoints = Locate_KeyPoint(DoG, SIFT_SIGMA, GuassianPyramid, n)  #LocateKeyPoint：关键点定位

    #3. 计算关键点的描述符
    discriptors = cal_Descriptors(GuassianPyramid, KeyPoints)  # calcDescriptors：计算描述符
...
def draw_Lines(X1, X2, Y1, Y2, dis, img, num=10):
...
def get_lower_quality(image_path, quality):
    ...
    # 打开降低质量后的图像
    # reduced_image = Image.open(image_path)
    reduced_image = plt.imread(out_path)
    # 返回降低质量后的图像对象
    return reduced_image


def get_sift_points(path_1, path_2):
    # ori_img = plt.imread('./1.jpg')  # 读第一张图片
    ori_img = get_lower_quality(path_1, 5)  # 读第一张图片
    if len(ori_img.shape) == 3:#如果是彩色图，就按照三通道取均值的方式转成灰度图
        img = ori_img.mean(axis=-1)
    else:
        img = ori_img
    keyPoints, discriptors = SIFT(img)  # 用SIFT算法计算关键点（x坐标，y坐标，sigma，主方向，梯度幅值）和描述符（128维的向量）

    # ori_img2 = plt.imread('./2.jpg')  # 读第二张图片
    ori_img2 = get_lower_quality(path_2, 5)  # 读第一张图片
    if len(ori_img.shape) == 3:
        img2 = ori_img2.mean(axis=-1)
    else:
        img2 = ori_img2
    ...
    ori_img2 = np.array(Image.fromarray(ori_img2).resize((img2.shape[1], img2.shape[0]), Image.BICUBIC))
    result = np.hstack((ori_img, ori_img2))
    ...
    Y1 = keyPoints[:, 0]
    Y2 = keyPoints2[:, 0]
    # draw_Lines(X1, X2, Y1, Y2, deltas, result) #把匹配的结果放到这里画线
